...
# Load subchronic
sub_chron <- read.csv("subchronic_tox_values.csv", header=T, stringsAsFactors=F, nrows=500, check.names=F)
...
category <- read.csv("air_tox_category_PBTs.csv", header=T, stringsAsFactors=F, nrows=500, check.names=F)
...
# SAVE RESULTS
write.csv(tox, "air_tox_endpoints.csv", row.names = FALSE)
